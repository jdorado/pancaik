...
@tool()
async def pipedrive_agent(
    data_store: Dict[str, Any],
    pipedrive: str,
    pipedrive_instructions: str
) -> Dict[str, Any]:
    """
    ...
    if not pipedrive:
        ai_logger.error(
            "No Pipedrive connection ID provided - cannot proceed with CRM operations",
            agent_id, account_id, agent_name
        )
    ...
    if db is None:
        ai_logger.error(
            "Database not initialized - cannot access Pipedrive connections",
            agent_id, account_id, agent_name
        )
    ...
    ai_logger.action(
        f"Retrieving Pipedrive connection details for connection ID: {pipedrive}",
        agent_id, account_id, agent_name
    )
    ...
    if not connection_params:
        ai_logger.error(
            f"Pipedrive connection '{pipedrive}' not found in database - please verify connection exists",
            agent_id, account_id, agent_name
        )
    ...
    if not api_token:
        ai_logger.error(
            "No API token found in Pipedrive connection parameters - authentication will fail",
            agent_id, account_id, agent_name
        )
        raise ValueError("API token not found in Pipedrive connection parameters")
    
    ai_logger.result(
        f"Successfully retrieved Pipedrive connection with API token",
        agent_id, account_id, agent_name
    )
    ...
    ai_logger.action(
        "Loading Pipedrive metadata (pipelines, stages, users) from cache or API",
        agent_id, account_id, agent_name
    )
    ...
    try:
        ...
        ai_logger.result(
            f"Successfully loaded Pipedrive metadata: {', '.join(metadata_summary)}",
            agent_id, account_id, agent_name
        )
    ...
    try:
        ...
        ai_logger.action(
            "Initializing Pipedrive client and creating tool calling capabilities",
            agent_id, account_id, agent_name
        )
        ...
        with PipedriveClient(api_token=api_token) as pipedrive_client:
            ...
            try:
                ...
                ai_logger.error(
                    "LangChain dependencies not installed - cannot create tool calling capabilities",
                    agent_id, account_id, agent_name
                )
...
CONTEXT: {context_json}"""
            ...
            if agent_result.get('error'):
                ai_logger.error(
                    f"AI agent execution failed: {agent_result.get('error')}",
                    agent_id, account_id, agent_name
                )
            ...
            ai_logger.result(
                f"AI agent completed successfully in {total_steps} steps",
                agent_id, account_id, agent_name
            )
            ...
            ai_logger.action(
                "Parsing and validating agent's JSON response for result quality",
                agent_id, account_id, agent_name
            )
            ...
            if error_message:
                ai_logger.error(
                    f"Agent validation failed: {error_message}",
                    agent_id, account_id, agent_name
                )
            ...
            if not results_found:
                ai_logger.result(
                    "No matching results found for the Pipedrive request - agent will exit",
                    agent_id, account_id, agent_name
                )
            ...
            if request_fulfilled:
                ...
                ai_logger.result(
                    f"Request fulfilled successfully: {data_summary}",
                    agent_id, account_id, agent_name
                )
            ...
            ai_logger.error(
                f"Results found but request could not be fulfilled - data quality: {data_quality}",
                agent_id, account_id, agent_name
            )
        ...
        ai_logger.error(
            f"Pipedrive agent execution failed: {str(e)}",
            agent_id, account_id, agent_name
        )
