...
async def load_and_cache_pipedrive_metadata(
    connection_handler: ConnectionHandler, connection_id: str, api_token: str, force_refresh: bool = False, cache_expiry_days: int = 7
) -> Dict[str, Any]:
    """
    ...
    with PipedriveClient(api_token=api_token) as client:
        try:
            ...
            cache_metadata = {
                ...
                "cache_items_count": {key: len(value) if isinstance(value, list) else 1 for key, value in cache_data.items()},
            }
...
async def get_cached_pipedrive_metadata(connection_handler: ConnectionHandler, connection_id: str, item_type: str = None) -> Dict[str, Any]:
    """
...
async def refresh_pipedrive_cache(connection_handler: ConnectionHandler, connection_id: str, api_token: str) -> Dict[str, Any]:
    """
    ...
    return await load_and_cache_pipedrive_metadata(
        connection_handler=connection_handler, connection_id=connection_id, api_token=api_token, force_refresh=True
    )
...
def simplify_pipedrive_metadata(cached_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "pipelines" in cached_metadata and "stages" in cached_metadata:
        ...
        for pipeline in pipelines:
            ...
            pipeline_stages = [stage for stage in stages if stage.get("pipeline_id") == pipeline_id]
            pipeline_stages.sort(key=lambda x: x.get("order_nr", 0))

            # Simplify stage information
            simplified_stages = [
                {"id": stage.get("id"), "name": stage.get("name"), "order": stage.get("order_nr", 0)} for stage in pipeline_stages
            ]

            simplified_pipelines[pipeline_name] = {"id": pipeline_id, "stages": simplified_stages}
