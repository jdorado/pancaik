...
async def create_phone_call(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post("https://api.retellai.com/v2/create-phone-call", headers=headers, json=payload) as response:
...
async def get_call_status(api_token: str, call_id: str) -> Dict[str, Any]:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.retellai.com/v2/get-call/{call_id}", headers=headers) as response:
