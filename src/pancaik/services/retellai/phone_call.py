...
def parse_us_phone_number(phone_number: str) -> str:
    ...
    if len(digits_only) == 10:
        ...
        raise ValueError(f"Invalid US phone number format: {phone_number}. Expected 10 digits (US number) or 11 digits starting with 1 (US number with country code). Got {len(digits_only)} digits: {digits_only}")
...
async def phone_call(
    ...
    ai_logger.action(
        f"Creating pretend phone call with instructions: {call_instructions[:100]}...", 
    ...
    ai_logger.result(
        f"Successfully created pretend phone call with ID: {mock_call_id}", 
        agent_id, account_id, agent_name
    )
    ...
    ai_logger.action(
        f"Creating phone call with instructions: {call_instructions[:100]}...", 
        agent_id, account_id, agent_name
    )
    ...
    retell_llm_dynamic_variables = {
        key: value for key, value in parsed_response.items() 
    ...
    ai_logger.result(
        f"Successfully created phone call with ID: {call_id}", 
        agent_id, account_id, agent_name
    )
