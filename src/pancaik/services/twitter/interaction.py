...
async def twitter_select_and_interact(
    ...
    ai_logger.action(f"Filtering {len(post_ids)} posts for previous interactions by {username}", agent_id, account_id, agent_name)

    # Get IDs of posts we've already interacted with
    processed_post_ids = await twitter_handler.get_posts_with_interactions(post_ids=post_ids, username=username)
    ...
    if not twitter_posts:
        ai_logger.result("All posts filtered out due to recent interactions with users", agent_id, account_id, agent_name)
    ...
    ai_logger.action(f"Checking interaction history with {len(usernames)} users", agent_id, account_id, agent_name)

    # Get last interaction dates for these usernames
    last_interactions = await twitter_handler.get_last_interactions_by_usernames(usernames=usernames, username=username)
    ...
    while start_idx < total_posts:
        ...
        if posts:
            ...
            if valid_posts:
                ...
                selected_tweet = next((post for post in batch if post["_id"] == top_tweet["post_id"]), None)
            ...
            for post in posts:
                ...
                if post_id and post_interaction_type == "ignore":
                    success = await twitter_handler.mark_post_interaction(post_id=post_id, username=username, interaction_type="ignored")
                    if success:
    ...
    if selected_tweet is None:
        ai_logger.result("No suitable posts found for interaction after checking all available posts", agent_id, account_id, agent_name)
        return {"should_exit": True}

    outputs = {"selected_tweet": selected_tweet, "interaction_type": interaction_type}
    ...
    ai_logger.action(f"Preparing {interaction_type} interaction with selected tweet", agent_id, account_id, agent_name)

    # Return in the required format for Pancaik tools
    return {
        "values": {"context": context, "output": outputs, "delete_context": ["twitter_posts"]},
    }
