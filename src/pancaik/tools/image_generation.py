...
async def image_generator(
    ...
    ai_logger.thinking(f"Starting image generation process. Type: {image_generation_type}", agent_id, account_id, agent_name)
    ...
    if "media_data" in context and context["media_data"]:
        ...
        for media_item in reversed(context["media_data"]):
            if media_item.get("mediaType", "").startswith("image/"):
                ...
                ai_logger.action(
                    f"Found image in context media_data, will use as input for image generation", agent_id, account_id, agent_name
                )
                break
    ...
    if image_generation_type == "contextual":
        ...
        contents = await _generate_contextual_content(
            final_context, final_image_style, image_prompt_guidelines, input_image, agent_id, account_id, agent_name, config
        )
            ...
            if candidate.content and candidate.content.parts:
                for part in candidate.content.parts:
                    if hasattr(part, "inline_data") and part.inline_data:
                        ...
                        generated_images.append({"data": image_byte_array, "mediaType": part.inline_data.mime_type or "image/jpeg"})
        ...
        if gemini_result.generated_images:
            for generated_image_obj in gemini_result.generated_images:
                ...
                generated_images.append({"data": image_byte_array, "mediaType": "image/jpeg"})
        ...
        try:
            grok_response = await grok_client.images.generate(
                model=grok_model_name, prompt=generated_grok_prompt, n=1, response_format="b64_json"
            )
            ...
            if grok_response.data:
                for img_data in grok_response.data:
                    if img_data.b64_json:
                        ...
                        generated_images.append({"data": image_byte_array, "mediaType": "image/png"})
        ...
        raise ValueError(
            f"Unknown image_generation_type: {image_generation_type}. Supported types: contextual, artistic, grok_image_generation."
        )

    if generated_images:
        ai_logger.result(
            f"Successfully generated {len(generated_images)} image(s) using {image_generation_type} model.",
            agent_id,
            account_id,
            agent_name,
        )
...
async def _describe_image_for_prompt(
    input_image: Image.Image, agent_id: str, account_id: str, agent_name: str, config: Dict[str, Any]
) -> str:
