...
async def topic_selector(data_store: Dict[str, Any], selection_guidelines: str) -> Dict[str, Any]:
    ...
    output_format = (
        ...
        '"full_background": "Fully detailed background information from context to use for composing the piece"\n'
    )
    prompt_data = {
        "task": "From the provided context and selection guidelines, select a single, unique, and focused topic to discuss. Also, extract the most relevant full background information from the context that should be used for composing an article, blog, post, or message. The goal is to focus the content creation on this topic and supporting info.",
