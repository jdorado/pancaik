...
def get_secret(service: str, account: str) -> Optional[str]:
    ...
    try:
        result = subprocess.run(
            ["security", "-i", "find-generic-password", "-s", service, "-a", account, "-w"], capture_output=True, text=True, check=True
        )
...
def delete_secret(service: str, account: str) -> bool:
    ...
    try:
        subprocess.run(["security", "-i", "delete-generic-password", "-s", service, "-a", account], capture_output=True, check=True)
        return True
    except subprocess.CalledProcessError as e:
